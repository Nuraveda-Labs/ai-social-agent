...
from social_signal.config import brand_config, settings
from social_signal.db.models import PublishedPost, ScheduledPost
from social_signal.db.session import _session_factory
from social_signal.integrations.google_sheets import update_row_by_key
from social_signal.sheet_posting.reader import SHEET_COLUMNS, QueuedPost
...
async def _post_as_quote_card(
    *,
    ...
    from social_signal.sheet_posting.quote_card import generate_quote_card
...
async def _post_linkedin_as_carousel(
    *,
    ...
    """
    from social_signal.media.carousel_gen import generate_carousel_from_body

    # Pick a sensible CTA link if the body contains one; fall back to org link
    cta_link = "github.com/<your-org>"
    for token in body.split():
        t = token.strip(".,!?")
        if "github.com/<your-org>" in t or "example.com" in t:
            cta_link = t
            break
...
async def _post_via_linkedin_native(
    *, row: QueuedPost, body: str, content_type: str,
    ...
    Author URN switches by brand:
      ...
      - glitch_executor  -> this project company URN (w_organization_social)
    ...
    Upload-Post path.
    """
    from social_signal.integrations.linkedin import (
        LinkedInError,
        author_urn_for,
