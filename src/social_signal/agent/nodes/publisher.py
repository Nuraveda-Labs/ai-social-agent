...
async def _ensure_local_file(asset: VideoAsset) -> None:
    ...
    from social_signal.db.models import ContentScript, Signal
    from social_signal.integrations import google_drive
...
async def _publish_to_platform(
    ...
    if platform == "youtube_shorts":
        from social_signal.platforms.youtube import upload_short
        return await upload_short(file_path, script_id, brand_id=brand_id)

    if platform == "tiktok":
        from social_signal.platforms.tiktok import publish as tiktok_publish
        return await tiktok_publish(file_path, script_id, brand_id=brand_id)

    if platform.startswith("zernio_"):
        from social_signal.platforms.zernio import publish as zernio_publish
        return await zernio_publish(platform, file_path, script_id, brand_id=brand_id)

    if platform.startswith("upload_post_"):
        from social_signal.platforms.upload_post import publish as upload_post_publish
    ...
    if platform.startswith("buffer_"):
        from social_signal.platforms.buffer import publish as buffer_publish
        return await buffer_publish(
            platform, file_path, script_id, brand_id=brand_id, attempts=attempts
        )

    if platform == "twitter":
        from social_signal.platforms.twitter import post_video
        return await post_video(file_path, script_id)

    if platform == "instagram_reels":
        from social_signal.platforms.instagram import post_reel
