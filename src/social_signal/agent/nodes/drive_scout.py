...
from social_signal.config import brand_config, resolve_publish_platform, settings
...
async def drive_scout_node(state: SignalAgentState) -> SignalAgentState:
    ...
    # in social_signal.config._PUBLISH_PRIORITY):
...
async def _mirror_to_sheet(brand_id: str, new_signals: list[dict]) -> None:
    ...
    from social_signal.media.filename_parser import parse as parse_filename
