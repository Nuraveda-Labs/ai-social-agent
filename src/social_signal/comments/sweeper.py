...
from social_signal.agent.nodes.text_writer import _forbidden_hits
from social_signal.config import brand_config, settings
from social_signal.db.models import CommentReply, PublishedPost
...
async def _draft_reply(
    *,
    ...
    from social_signal.agent.nodes.text_writer import _x_specific_hits
...
async def _send_approval_message(row: CommentReply) -> None:
    ...
    from social_signal.discord.formatter import comment_reply_embed
    from social_signal.discord.rest import add_reaction, post_message
...
async def approve_reply(comment_reply_id: str) -> tuple[bool, str]:
    ...
    try:
        if row.platform == "upload_post_x":
            ...
            from social_signal.integrations.x import XClient
